...
<section class="wrapper">
  <div class="breadcrumb margin-bottom-10">
    ...
    <div class="bar-nav pull-right ">
      <a id="modal-button" class="btn btn-default <?php echo $status_c?>" onclick='addForm("<?php echo base_url('company/add_bank'); ?>");' role="button" data-toggle="modal" data-target="#myModalLg" data-backdrop="static">
          <i class="fa fa-file-o fa-fw"></i> Add Bank
      </a>
    </div>
    <!-- </li> -->
  </div>
  <div class="col-lg-12 padding-left-right-10">
    <div class="panel margin-bottom-10">
      <div class="panel-heading"><i class='fa fa-factory'></i> List Bank
          <span class="tools pull-right">
              <a class="fa fa-chevron-up" href="javascript:;"></a>
           </span>
      </div>
      <div class="panel-body panel-body-border" style="display: block;">
        <form id="filterForm" action="<?php echo base_url('company/bank') ?>" class="bucket-form" method="get">
          <div id="ajax-url" url="<?php echo base_url('company/bank_typeahead');?>"></div>
          <div class="row">
               ...
               <div class="form-group">
                  ...
                  <select name="kd_dealer" id="kd_dealer" class="form-control">
                     ...
                     foreach ($dealer->message as $key => $group) : 
                        $default=($defaultDealer==$group->KD_DEALER)?" selected":" ";
                        ?>
         ...
         ?>
            <tr id="<?php echo $this->session->flashdata('tr-active') == $row->ID ? 'tr-active' : ' ';?>" >
              ...
              <td class="table-nowarp">
                <a id="modal-button" onclick='addForm("<?php echo base_url('company/edit_bank/'.$row->KD_BANK.'/'.$row->ROW_STATUS); ?>");' role="button" data-toggle="modal" data-target="#myModalLg" data-backdrop="static" class="<?php echo $status_v?>">
                  <i data-toggle="tooltip" data-placement="left" title="Edit" class="fa fa-edit text-success text-active"></i>
                </a>
            <?php 
                                 if($row->ROW_STATUS == 0){ 
                                 ?>
                <a id="delete-btn<?php echo $no;?>" class="delete-btn <?php echo $status_e?>" url="<?php echo base_url('company/delete_bank/'.$row->KD_BANK); ?>">
