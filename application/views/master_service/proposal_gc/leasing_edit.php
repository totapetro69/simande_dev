  ...
  if($list->message[0]->KD_LEASING != "ALL"){
    ...
    <div class="form-group" id="leasing">
      ...
      <?php if($leasing && (is_array($leasing->message) || is_object($leasing->message))): foreach ($leasing->message as $key => $value) : ?>
        <input type="checkbox" name="leasing[]" value="<?php echo $value->KD_LEASING;?>" <?php if($list && (is_array($list->message) || is_object($list->message))): foreach($list->message as $key=>$row): echo ($value->KD_LEASING == $row->KD_LEASING ? "checked=checked" : ""); endforeach; endif;?>> <?php echo $value->KD_LEASING;?> - <?php echo $value->NAMA_LEASING;?><br>
