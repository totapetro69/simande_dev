...
$paramtgl =($this->input->get("start_date"))?"&start_date=".$this->input->get("start_date")."&end_date=".$this->input->get("end_date"):"";
?>
...
<section class="wrapper">
  <div class="breadcrumb margin-bottom-10">
    ...
    <div class="bar-nav pull-right ">
      <a class="btn btn-default  <?php echo $status_p ?>" id="modal-button" onclick='addForm("<?php echo base_url('report/lap_pinjaman_bpkb_print?kd_dealer=' . $defaultDealer . $paramtgl); ?>");'  role="button" data-toggle="modal" data-target="#myModalLg" data-backdrop="static">
  ...
  <div class="col-lg-12 padding-left-right-10">
    <div class="panel margin-bottom-10">
      ...
      <div class="panel-heading"><i class='fa fa-list-ol'></i> Laporan Pinjaman BPKB
      ...
      <div class="panel-body panel-body-border" >
        <form id="filterFormz" action="<?php echo base_url('report/lap_pinjaman_bpkb') ?>" class="bucket-form" method="get">
          <div class="row">
              ...
              if (is_array($list->message) || is_object($list->message)):
                foreach ($list->message as $key => $row):
                  ...
                  <tr id="<?php echo $this->session->flashdata('tr-active') == $row->NO_TRANS ? 'tr-active' : ' '; ?>" >
                    <td><?php echo $no; ?></td>
                    <td style="white-space: nowrap;"><?php echo $row->NO_TRANS; ?></td>
                    <td><?php echo tglFromSql($row->TGL_PINJAM); ?></td>
                    <td><?php echo $row->NO_MESIN; ?></td>
                    <td><?php echo $row->NAMA_PEMILIK; ?></td>
                    <td class='text-right table-nowarp'><?php echo number_format($row->BIAYA_BPKB,0); ?></td>
                    <td style="white-space: nowrap;"><?php echo ((int)$row->REQ_ADMIN_SAMSAT==2)?'ADMIN SAMSAT, ':'';?>
        ...
        <div class="row">
          ...
          <div class="col-sm-5">
            <small class="text-muted inline m-t-sm m-b-sm"> 
              <?php echo ($list) ? ($list->totaldata == 0 ? "" : "<i>Total Data " . $list->totaldata . " items</i>") : '' ?>
            </small>
          </div>
