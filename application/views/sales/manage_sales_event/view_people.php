  ...
  <div class="breadcrumb margin-bottom-10">
    ...
    <div class="bar-nav pull-right ">
      ...
      if($cek->message[0]->APPROVAL_MD != 1){ 
        ?>
        <a id="modal-button" class="btn btn-default <?php echo $status_c?>" onclick='addForm("<?php echo base_url('sales_event/add_people/'. $cek->message[0]->ID.'/'.$cek->message[0]->KD_EVENT); ?>");' role="button" data-toggle="modal" data-target="#myModalLg" data-backdrop="static">
                  ...
                  foreach($list->message as $key=>$row):
                    ...
                    <tr id="<?php echo $this->session->flashdata('tr-active') == $row->ID ? 'tr-active' : ' ';?>" >
                      ...
                      <td class="table-nowarp">
                        ...
                        if($cek->message[0]->APPROVAL_MD != 1){ 
                          ?>
                          <a id="modal-button" onclick='addForm("<?php echo base_url('sales_event/edit_people/'.$row->ID.'/'.$row->ROW_STATUS); ?>");' role="button" data-toggle="modal" data-target="#myModalLg" data-backdrop="static" class="<?php echo $status_v?>">
                        ...
                        if($row->ROW_STATUS == 0){ 
                          if($cek->message[0]->APPROVAL_MD != 1){ 
                            ?>
                            <a id="delete-btn<?php echo $no;?>" class="delete-btn" url="<?php echo base_url('sales_event/delete_people/'.$row->ID.'/'.$row->KD_EVENT); ?>">
                      ...
                      <td><?php echo $row->KEHADIRAN_SALES;?></td>
                    </tr>
